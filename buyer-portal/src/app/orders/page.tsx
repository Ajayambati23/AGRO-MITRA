...
import { marketplaceAPI, ordersAPI, Order, getErrorMessage } from '@/lib/api';
...
export default function MyOrdersPage() {
  ...
  const listing = (o: Order) => (o.listingId as { cropName?: string; unit?: string; pricePerUnit?: number; sellerId?: { name?: string; phone?: string; email?: string } }) || {};
  const listingName = (o: Order) => listing(o).cropName || 'Crop';
  const listingUnit = (o: Order) => listing(o).unit || '';
  const seller = (o: Order) => listing(o).sellerId || {};
  const orderIdShort = (id: string) => id ? `#${String(id).slice(-8).toUpperCase()}` : '';
  const listingId = (o: Order) => (o.listingId as { _id?: string })?._id || '';

  const submitReview = async (o: Order) => {
    const oid = o._id;
    ...
    try {
      await marketplaceAPI.createReview(lid, state.rating, state.comment);
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => loadOrders()}
              disabled={loading}
              className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-white text-sm"
            >
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
            ...
            <Link href="/" className="inline-block mt-4 px-6 py-2 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700">
              Browse listings
            </Link>
          </div>
        ) : (
                ...
                <div key={o._id} className="bg-white rounded-2xl border border-gray-200 p-5">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    ...
                    <div className="flex flex-wrap gap-3 text-sm">
                      ...
                      {farmer.phone && (
                        <a href={`tel:${farmer.phone}`} className="flex items-center gap-1.5 text-green-600 hover:text-green-700 font-medium">
                          <Phone className="w-4 h-4" /> {farmer.phone}
                        </a>
                      )}
                      {farmer.email && (
                        <a href={`mailto:${farmer.email}`} className="flex items-center gap-1.5 text-green-600 hover:text-green-700 font-medium">
                          <Mail className="w-4 h-4" /> {farmer.email}
                        </a>
                      )}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      ...
                      {reviewByOrder[o._id]?.done ? (
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <select
                            value={reviewByOrder[o._id]?.rating ?? 5}
                            onChange={(e) => setReviewByOrder((p) => ({ ...p, [o._id]: { ...(p[o._id] || { comment: '' }), rating: Number(e.target.value) } }))}
                          ...
                          <input
                            ...
                            value={reviewByOrder[o._id]?.comment ?? ''}
                            onChange={(e) => setReviewByOrder((p) => ({ ...p, [o._id]: { ...(p[o._id] || { rating: 5 }), comment: e.target.value } }))}
                            className="px-3 py-2 rounded-lg border border-gray-200 text-sm min-w-[220px] flex-1"
                          />
                          <button
                            type="button"
                            onClick={() => submitReview(o)}
                            disabled={reviewByOrder[o._id]?.saving}
