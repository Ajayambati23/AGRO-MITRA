...
import { marketplaceAPI, ordersAPI, getErrorMessage, Listing } from '@/lib/api';
...
export default function BuyerPortalPage() {
  ...
  // Auto-refresh listings every 30s so new listings appear
  useEffect(() => {
  ...
  const handleProceed = async (listing: Listing) => {
    const q = quantityByListing[listing._id];
    const num = typeof q === 'number' ? q : 0;
    if (num <= 0 || num > listing.quantity) {
      setError(`Enter quantity between 1 and ${listing.quantity}`);
      return;
    }
    setError(null);
    setOrderSuccess(null);
    setProceedingId(listing._id);
    try {
      const { order } = await ordersAPI.create(listing._id, num);
      ...
      setQuantityByListing((prev) => ({ ...prev, [listing._id]: 0 }));
  ...
  const locationStr = (l: Listing) => {
    const loc = l.location || (l.sellerId as { location?: Record<string, string> })?.location;
    if (!loc) return null;
    const parts = [loc.village, loc.district, loc.state].filter(Boolean);
    return parts.length ? parts.join(', ') : loc.state || null;
  };

  const sellerName = (l: Listing) => (l.sellerId as { name?: string })?.name || 'Farmer';
          ...
          <div className="flex items-center gap-3">
            {isLoggedIn ? (
              <>
                ...
                <Link
                  href="/orders"
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium"
                >
              ...
              <>
                <Link href="/login" className="px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium">
                  Login
                </Link>
                <Link href="/register" className="px-4 py-2 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-medium">
                  Register
                </Link>
              </>
            )}
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        <form onSubmit={handleSearch} className="flex flex-wrap gap-3 mb-8">
          <input
            ...
            className="flex-1 min-w-[200px] px-4 py-3 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
          />
          <select
            value={stateFilter}
            onChange={(e) => setStateFilter(e.target.value)}
            className="px-4 py-3 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-green-500"
          >
          ...
          <button
            ...
            onClick={() => load()}
            disabled={loading}
            className="px-6 py-3 bg-slate-600 text-white font-medium rounded-xl hover:bg-slate-700 disabled:opacity-50 flex items-center gap-2"
          >
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl">
            {error}. {!error.includes('quantity') && 'Ensure the AgroMitra backend is running on port 3001.'}
          </div>
        )}
                ...
                >
                  ...
                  {locationStr(listing) && (
                    <p className="text-xs text-gray-500 flex items-center gap-1 mb-3">
                      <MapPin className="w-3 h-3" /> {locationStr(listing)}
                    </p>
                  )}
                  <div className="mt-auto pt-4 border-t border-gray-100 space-y-3">
                      ...
                      <>
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            step={0.1}
                            value={quantityByListing[listing._id] ?? ''}
                            onChange={(e) => setQuantityByListing((prev) => ({ ...prev, [listing._id]: parseFloat(e.target.value) || 0 }))}
                        ...
                        <button
                          type="button"
                          onClick={() => handleProceed(listing)}
                          disabled={proceedingId === listing._id}
                          className="w-full py-2 px-4 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 disabled:opacity-50 text-sm"
                        >
                          {proceedingId === listing._id ? 'Placing...' : 'Proceed to buy'}
                        </button>
                      </>
                    ) : (
                      <Link href="/login" className="block w-full py-2 px-4 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 text-center text-sm">
                        Sign in to buy
                      </Link>
                    )}
                    ...
                    {listing.sellerRating && (
                      <p className="text-xs text-amber-700">
                        Rating: {listing.sellerRating.avgRating ?? 'New'} {listing.sellerRating.avgRating ? '/ 5' : ''} ({listing.sellerRating.totalReviews} review{listing.sellerRating.totalReviews === 1 ? '' : 's'})
                      </p>
                    )}
                    <p className="text-xs text-gray-400">Contact details visible after purchase (My Orders)</p>
                  </div>
                </div>
              ))}
            </div>
